...
export const AdminCounselorsPage: React.FC = () => {
  ...
  const [targetDirector, setTargetDirector] = useState<User | null>(null);
  ...
  const fetchDirectors = async () => {
    try {
      setIsLoading(true);
      const res = await api.get('/users?role=DIRECTOR');
      setDirectors(res.data.data || []);
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to load Directors list');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenToggleModal = (dir: User) => {
    setTargetDirector(dir);
    setSuperAdminPassword('');
  };

  const handleConfirmToggle = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetDirector || !superAdminPassword.trim()) {
      toast.error('Super Administrator authorization password is required.');
      return;
    }

    const nextStatus = !targetDirector.is_counselor;

    try {
      setIsSubmitting(true);
      const res = await api.patch(`/counseling/${targetDirector.id}/counselor-status`, {
        isCounselor: nextStatus,
        superAdminPassword: superAdminPassword.trim()
      });

      toast.success(res.data.message || `Director ${targetDirector.name} counselor status updated.`);
  ...
  const appointedCount = directors.filter((d) => d.is_counselor).length;
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="relative overflow-hidden bg-slate-900 p-5 rounded-2xl text-white shadow-md border border-slate-800">
        ...
        <div className="relative z-10 space-y-1.5">
          ...
          <p className="text-xs text-slate-300 font-medium max-w-2xl">
            Appoint active Directors as <strong className="text-rose-300">Mental Health Counseling Teachers</strong>. Super Administrator password verification is required to authorize counselor appointments.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-rose-600">{appointedCount} of {directors.length} Directors Appointed</p>
          </div>
        </div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            placeholder="Search directors by name or department..."
      ...
      {isLoading ? (
        <LoadingState message="Loading Directors list..." />
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center space-y-2 shadow-2xs">
          ...
          <h4 className="text-xs font-black text-slate-900 uppercase">No Directors Found</h4>
          <p className="text-xs text-slate-500 max-w-xs mx-auto">Create Director accounts first to appoint them as Mental Health Counseling Teachers.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button onClick={() => setTargetDirector(null)} className="p-1 text-slate-400 hover:text-slate-600 cursor-pointer">
            ...
            <form onSubmit={handleConfirmToggle} className="space-y-3.5 text-xs font-semibold">
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl space-y-1">
                <p className="text-[10px] font-black text-rose-900 uppercase tracking-wider">Target Director:</p>
                <h4 className="font-extrabold text-xs text-rose-950">{targetDirector.name} ({getBranchShortCode(targetDirector.department)})</h4>
                <p className="text-[11px] text-rose-800 font-medium">
                  New Status: <strong className="text-rose-950">{!targetDirector.is_counselor ? 'Appointed Mental Health Counselor Teacher' : 'Regular Director Only'}</strong>
                </p>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setTargetDirector(null)}
