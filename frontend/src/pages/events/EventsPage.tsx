...
export const EventsPage: React.FC = () => {
  ...
  const paginatedEvents = events.slice((safeCurrentPage - 1) * pageSize, safeCurrentPage * pageSize);
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-semibold text-slate-700">
            <div className="flex items-center gap-3">
              <span className="text-slate-500 font-bold">
                Showing <span className="text-slate-900 font-black">{(safeCurrentPage - 1) * pageSize + 1}</span> to <span className="text-slate-900 font-black">{Math.min(safeCurrentPage * pageSize, events.length)}</span> of <span className="text-slate-900 font-black">{events.length}</span> events
              </span>
            </div>
